...
def _validate_node_contract(
    ...
    if len(node.get("outputs", [])) != 1:
        raise ValueError("avg_pool2d expects exactly one output")
    dtype = str(output_tensor["dtype"])
    if dtype not in POOLING_DTYPES:
        raise NotImplementedError(f"avg_pool2d lowering does not support dtype {output_tensor['dtype']}")
    if str(input_tensor["dtype"]) != dtype:
        raise ValueError("avg_pool2d input and output dtype must match")
    attrs = node.get("attrs", {})
    kernel, stride, padding = normalize_avg_pool2d_attrs(
        attrs.get("kernel_size"),
        attrs.get("stride"),
        attrs.get("padding", (0, 0)),
    )
    expected_shape = resolve_avg_pool2d_shape(input_tensor["shape"], kernel, stride, padding)
    if list(expected_shape) != list(output_tensor["shape"]):
        raise ValueError("avg_pool2d output shape does not match attrs")
...
AVG_POOL2D_LOWERING = OpLowering(
    op_name="avg_pool2d",
