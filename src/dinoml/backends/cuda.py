...
def _ensure_cmake_cutlass_gemm_archives(arch: str, kernel_manifest: Mapping[str, Any]) -> tuple[Path, ...]:
    ...
    build_profiler_bindings = os.environ.get("DINOML_BUILD_CUTLASS_GEMM_PROFILERS") == "1"
    profiler_link_targets = [module["target"] for module in modules]
    profiler_bind_targets = [
        f"dinoml_cutlass_gemm_profiler_{module['op']}_{module['dtype']}_bind"
    ...
    if any(not archive.exists() for archive in archives) or not build_dir.exists() or build_profiler_bindings:
        ...
        if build_profiler_bindings and profiler_link_targets:
            configure_args.append(
                "-DDINOML_CUTLASS_GEMM_PROFILER_TARGETS=" + ";".join(profiler_link_targets)
            )
        _run_cmake(
            configure_args,
            cwd=repo_root,
