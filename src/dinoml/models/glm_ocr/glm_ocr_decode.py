...
_CACHE_VARIANTS = frozenset({"none", "static", "session"})
...
def build_spec(
    *,
    ...
    batch_dim = Dim(
        ...
        typical=settings.batch,
        buckets=_equal_interval_buckets(settings.min_batch, settings.max_batch, count=settings.batch_bucket_count),
    )
    past_len_dim = Dim(
        "past_len",
        min=settings.min_past_len,
        max=settings.max_past_len,
        typical=settings.past_len,
        buckets=_equal_interval_buckets(settings.min_past_len, settings.max_past_len, count=settings.past_bucket_count),
    )
    ...
    inputs: dict[str, dml.TensorSpec] = {"input_ids": dml.TensorSpec([batch_dim, 1], "int64")}
    model = None
    if variant == "session":
        model = GlmOcrForConditionalGenerationDecodeSessionStaticCache(
            config,
            weights,
            max_cache_len=settings.max_cache_len,
        )
        ...
        if use_attention_mask:
            inputs["attention_mask"] = dml.TensorSpec(
                [batch_heads_dim, 1, settings.max_cache_len],
                config.text_config.dtype,
            )
        spec_name = (
            "glm_ocr_official_decode_dynamic_session_static_cache_"
            f"b{settings.max_batch}_cache{settings.max_cache_len}"
        )
    else:
        inputs["cos"] = dml.TensorSpec([batch_dim, 1, config.text_config.head_dim], config.text_config.dtype)
        inputs["sin"] = dml.TensorSpec([batch_dim, 1, config.text_config.head_dim], config.text_config.dtype)
        ...
        for layer_idx in range(config.text_config.num_hidden_layers):
            ...
            inputs[f"past_key_{layer_idx}"] = dml.TensorSpec(
                [batch_dim, config.text_config.num_key_value_heads, buffer_len, config.text_config.head_dim],
                config.text_config.dtype,
            )
        ...
        if variant == "static":
            ...
            spec_name = (
                "glm_ocr_official_decode_dynamic_static_cache_"
                f"b{settings.max_batch}_past{settings.max_past_len}"
            )
        else:
            model = GlmOcrForConditionalGenerationDecode(config, weights)
            spec_name = f"glm_ocr_official_decode_dynamic_b{settings.max_batch}_past{settings.max_past_len}"
    spec = dml.trace(model, inputs=inputs, name=spec_name)
    return attach_profiling_metadata(
        spec,
        _profiling_shape_scenarios(
            settings,
            config,
...
def build_validation_inputs(
    *,
    ...
    inputs = {
        "input_ids": np.full((batch_size, 1), 42, dtype=np.int64),
        "cos": _float_input(
            np.ones((batch_size, 1, config.text_config.head_dim), dtype=np.float32),
            config.text_config.dtype,
        ),
        "sin": _float_input(
            np.zeros((batch_size, 1, config.text_config.head_dim), dtype=np.float32),
            config.text_config.dtype,
        ),
    }
    if variant == "none" or use_attention_mask:
    ...
    if variant == "session":
        return inputs
    if variant == "static":
        inputs["cache_seqlens"] = np.full((batch_size,), settings.past_len, dtype=np.int32)
    ...
    for layer_idx in range(config.text_config.num_hidden_layers):
        inputs[f"past_key_{layer_idx}"] = _float_input(
            rng.normal(
                0.0,
                0.01,
                (batch_size, config.text_config.num_key_value_heads, cache_buffer_len, config.text_config.head_dim),
            ).astype(np.float32),
            config.text_config.dtype,
        )
        ...
        for batch_size in batch_values:
            ...
            if use_attention_mask:
                overrides["attention_mask"] = [
                    ...
                    1,
                    settings.max_cache_len,
                ]
            scenarios.append(
                {
                    "source": "workflow_equal_interval_buckets",
                    "case_id": f"batch={int(batch_size)}_cache_len={settings.max_cache_len}",
                    "dim_values": {"batch": int(batch_size), "max_cache_len": settings.max_cache_len},
                    "overrides": overrides,
                }
            )
        return scenarios
    past_values = _equal_interval_buckets(
        settings.min_past_len,
        settings.max_past_len,
        count=settings.past_bucket_count,
    )
    for batch_size in batch_values:
...
def _resolve_settings(
    *,
    ...
    resolved_max_cache_len = (
        max(resolved_max_past_len + 1, 2) if max_cache_len is None else int(max_cache_len)
    )
    resolved_batch_bucket_count = int(batch_bucket_count)
    resolved_past_bucket_count = int(past_bucket_count)
    if resolved_min_batch <= 0 or resolved_batch <= 0 or resolved_max_batch <= 0:
        raise ValueError("batch bounds must be positive")
    if resolved_min_batch > resolved_batch or resolved_batch > resolved_max_batch:
        raise ValueError("expected min_batch <= batch <= max_batch")
    if resolved_min_past_len <= 0 or resolved_past_len <= 0 or resolved_max_past_len <= 0:
        raise ValueError("past_len bounds must be positive")
    if resolved_min_past_len > resolved_past_len or resolved_past_len > resolved_max_past_len:
        raise ValueError("expected min_past_len <= past_len <= max_past_len")
    if resolved_max_cache_len <= resolved_max_past_len:
        raise ValueError("max_cache_len must be greater than max_past_len")
    if resolved_batch_bucket_count <= 0 or resolved_past_bucket_count <= 0:
    ...
    return WorkflowSettings(
        ...
        dtype=str(dtype),
        batch_bucket_count=resolved_batch_bucket_count,
        past_bucket_count=resolved_past_bucket_count,
    )
