...
class MaxPool2dModule(dml.Module):
    ...
    def forward(self, x):
        return dml.ops.output(dml.ops.max_pool2d(x, self.kernel_size, self.stride, self.padding), "out")


def _trace_max_pool2d(dtype="float32", kernel_size=(2, 3), stride=None, padding=(1, 0), shape=(2, 3, 5, 7)):
    return dml.trace(
        MaxPool2dModule(kernel_size, stride, padding),
        inputs={"x": dml.TensorSpec(shape, dtype)},
        name=f"max_pool2d_{dtype}",
    )


def _input(shape, dtype):
    value = np.linspace(-7.0, 5.0, num=int(np.prod(shape)), dtype=np.float32).reshape(shape)
...
def _np_max_pool2d(x, kernel_size, stride=None, padding=0):
...
def test_max_pool2d_frontend_ir_defaults_stride_to_kernel_and_preserves_dtype():
    spec = _trace_max_pool2d("float32", kernel_size=(2, 3), stride=None, padding=(1, 2), shape=(2, 3, 5, 7))
...
def test_cpu_reference_max_pool2d_uses_negative_infinity_padding():
    shape = (1, 1, 1, 1)
    ...
    assert actual[0, 0, 2, 2] == np.float32(3.5)
    np.testing.assert_allclose(actual, expected, rtol=0.0, atol=0.0)


@pytest.mark.parametrize("dtype", ["float32", "float16", "bfloat16"])
def test_max_pool2d_generated_cpu_source_and_runtime(tmp_path, dtype):
    shape = (1, 2, 1, 1)
    spec = _trace_max_pool2d(dtype, kernel_size=1, stride=1, padding=2, shape=shape)
    lowered, _ = PassManager().run(spec.ir)
    validate_ir(lowered)
    assert [node["op"] for node in lowered["nodes"]] == ["max_pool2d"]
    tensor_map = {tensor["name"]: tensor for tensor in lowered["tensors"]}

    cpu_source = render_generated_kernels("cpu", lowered["nodes"], tensor_map)[0]

    assert "static int max_pool2d_" in cpu_source
    assert "float max_value = -INFINITY" in cpu_source
    assert "const float value = dinoml::math::cast<float>(x[input_idx])" in cpu_source
    assert "y[idx] = dinoml::math::cast<" in cpu_source
    if dtype == "float32":
        assert "const float* DINO_RESTRICT x" in cpu_source
        assert "float* DINO_RESTRICT y" in cpu_source

    artifact = dml.compile(spec, dml.Target("cpu"), tmp_path / f"max_pool2d_{dtype}_cpu.dinoml")
    ...
    expected = _storage_roundtrip(_np_max_pool2d(x, 1, stride=1, padding=2), dtype)
    assert np.isneginf(actual[0, 0, 0, 0])
    np.testing.assert_allclose(actual, expected, rtol=0.0, atol=0.0)


def test_max_pool2d_generated_cuda_source_supports_reduced_precision():
    for dtype, pointer_type in (
        ("float16", "const half* DINO_RESTRICT x"),
        ("bfloat16", "const __nv_bfloat16* DINO_RESTRICT x"),
    ):
        spec = _trace_max_pool2d(dtype, kernel_size=(3, 2), stride=(2, 1), padding=(1, 0), shape=(1, 2, 5, 4))
        lowered, _ = PassManager().run(spec.ir)
        tensor_map = {tensor["name"]: tensor for tensor in lowered["tensors"]}
        ...
        assert "float max_value = -INFINITY" in cuda_source
        assert "const float value = dinoml::math::cast<float>(x[input_idx])" in cuda_source
        assert "y[idx] = dinoml::math::cast<" in cuda_source


def test_max_pool2d_frontend_rejects_dynamic_rank_dtype_and_bad_attrs():
    class DynamicMaxPool(dml.Module):
        def forward(self, x):
            return dml.ops.max_pool2d(x, 2)

    with pytest.raises(ValueError, match="only static input shapes"):
        dml.trace(DynamicMaxPool(), inputs={"x": dml.TensorSpec([1, 3, Dim("h", 4, 8), 8])})
    with pytest.raises(ValueError, match="rank-4"):
        _trace_max_pool2d("float32", shape=(2, 3, 4))
    with pytest.raises(ValueError, match="does not support dtype bool"):
        _trace_max_pool2d("bool")
    with pytest.raises(ValueError, match="positive integers"):
        _trace_max_pool2d("float32", kernel_size=0)
    with pytest.raises(ValueError, match="positive integers"):
        _trace_max_pool2d("float32", kernel_size=2, stride=(1, 0))
    with pytest.raises(ValueError, match="non-negative integers"):
        _trace_max_pool2d("float32", kernel_size=2, padding=(0, -1))
    with pytest.raises(ValueError, match="pair of integers"):
        _trace_max_pool2d("float32", kernel_size=(2, 3, 4))
    with pytest.raises(ValueError, match="non-bool integers"):
        _trace_max_pool2d("float32", kernel_size=(True, 2))
    with pytest.raises(ValueError, match="output height must be positive"):
        _trace_max_pool2d("float32", kernel_size=(8, 2), shape=(1, 1, 4, 4))


def test_max_pool2d_validation_rejects_dynamic_shape_spec_bad_attrs_shape_and_dtype():
    spec = _trace_max_pool2d("float32", kernel_size=2, stride=1, padding=0)
