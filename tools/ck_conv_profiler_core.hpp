...
inline std::vector<ConvResult> profile_conv(const ConvRequest& request, std::uint32_t seed) {
  ...
  for (int repeat = 0; repeat < request.repeats; ++repeat) {
    const float elapsed_ms = run_candidate(request, x.get(), weight.get(), bias.get(), residual.get(), output.get());
    if (!(elapsed_ms >= 0.0f)) {
      throw std::runtime_error("CK Conv profiler candidate failed: " + request.profiler_symbol);
    }
