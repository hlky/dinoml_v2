...
- [ ] Creation/shape values: `arange`, `randn`, `meshgrid`. `full` is now
  available for non-empty positive static dense shapes with `float32`,
  `float16`, `bfloat16`, and `bool` storage, using CPU reference execution plus
  generated CPU/CUDA fill kernels. Dynamic shapes and integer dtypes remain out
  of scope for this bounded port. `cast` is available for dense tensor casts
  across the current generated float/reduced-precision/bool storage surface.
