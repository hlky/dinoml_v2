...
class Conv2dBiasModule(dml.Module):
    def __init__(self, op_name: str = "conv2d_bias"):
        self.op_name = op_name
...
class Conv2dBiasAddReluModule(dml.Module):
    def forward(self, x, weight, bias, residual):
        return dml.ops.output(
            dml.ops.conv2d_bias_add_relu(
                x,
                weight,
                bias,
                residual,
...
def _conv2d_profile_workload(op_name: str = "conv2d_bias"):
    spec = dml.trace(
        Conv2dBiasModule(op_name),
        inputs={
            "x": dml.TensorSpec([2, 3, 7, 8], "float16"),
            "weight": dml.TensorSpec([4, 3, 3, 2], "float16"),
            "bias": dml.TensorSpec([4], "float16"),
        },
        name=f"profile_{op_name}_scaffold",
    )
...
def _conv2d_bias_add_relu_profile_workload():
    spec = dml.trace(
        ...
        name="profile_conv2d_bias_add_relu_scaffold",
    )
    kernel_manifest = build_kernel_manifest(spec.ir, DEFAULT_CUDA_TARGET)
    workloads = build_profile_workloads(spec.ir, kernel_manifest)
    workload = next(item for item in workloads if item.candidate_id == kernel_manifest["required_kernels"][0]["selected_candidate_id"])
...
def test_cutlass_conv_profile_cache_key_write_and_read_are_conv_specific(tmp_path):
    ...
    timing = _profile_timing([0.25, 0.25, 0.25], iterations=5)
    ...
    cache_entry = _cache_entry(workload, result, key_payload)
    cached_result = _profile_result_from_cache(workload, cache_entry)

    assert key_payload["kernel_library"] == "cutlass_conv"
    assert key_payload["layout_translation"] == dict(workload.layout_translation)
    assert key_payload["weight_transform"] == dict(workload.weight_transform)
    assert key_payload["conv_config"] == dict(workload.conv_config)
    assert "alignment_context" not in cache_entry
    assert cache_entry["layout_translation"] == dict(workload.layout_translation)
    assert cache_entry["weight_transform"] == dict(workload.weight_transform)
    assert cache_entry["conv_config"] == dict(workload.conv_config)
    assert cached_result["status"] == "cached"
    assert cached_result["kernel_library"] == "cutlass_conv"
    assert cached_result["shape"]["n"] == 2
    assert cached_result["selected"]["candidate_id"] == workload.candidate_id


def test_build_profile_workloads_supports_cutlass_conv_bias_relu_epilogue(tmp_path):
    workload, kernel_manifest = _conv2d_profile_workload("conv2d_bias_relu")
    codegen_plan = create_codegen_plan(kernel_manifest, tmp_path / "cache").to_json()
    ...
    assert kernel_manifest["required_kernels"][0]["cutlass_conv_plan"]["epilogue_config"] == {"inputs": ["bias"], "activation": "relu"}
    ...
    assert key_payload["op"] == "conv2d_bias_relu"
...
def test_build_profile_workloads_supports_cutlass_conv_bias_add_relu_epilogue(tmp_path):
    workload, kernel_manifest = _conv2d_bias_add_relu_profile_workload()
    codegen_plan = create_codegen_plan(kernel_manifest, tmp_path / "cache").to_json()
    ...
    assert key_payload["op"] == "conv2d_bias_add_relu"
    assert key_payload["candidate_set_id"] == "cutlass_conv_conv2d_bias_add_relu_float16_nhwc_ohwi_bias_add_relu_v1"
    assert "bias_add_relu" in key_payload["kernel_symbol"]
    assert "bias_add_relu" in key_payload["profiler_symbol"]


def test_cutlass_conv_static_execution_plan_updates_manifest_and_conv_plan():
    _workload, kernel_manifest = _conv2d_bias_profile_workload()
    workloads = build_profile_workloads(
        dml.trace(
            ...
            name="profile_conv2d_bias_static_plan",
        ).ir,
        kernel_manifest,
    )
...
def test_cutlass_conv_bias_relu_static_execution_plan_updates_manifest_and_conv_plan():
    _workload, kernel_manifest = _conv2d_bias_relu_profile_workload()
    workloads = build_profile_workloads(
        dml.trace(
            ...
            name="profile_conv2d_bias_relu_static_plan",
        ).ir,
        kernel_manifest,
    )
    simt = next(item for item in workloads if item.candidate["selection_predicate"]["kind"] == "fallback")
    ...
    assert plan["summary"]["static_selection_count"] == 1
    assert plan["static_selections"][0]["kernel_library"] == "cutlass_conv"
    assert plan["static_selections"][0]["op"] == "conv2d_bias_relu"
    assert plan["static_selections"][0]["selected_candidate_id"] == simt.candidate_id
    assert "bias_relu" in plan["static_selections"][0]["kernel_symbol"]
    assert "bias_relu" in plan["static_selections"][0]["profiler_symbol"]
    ...
    assert required["profiler_symbol"] == simt.profiler_symbol
    assert required["execution_plan_selection"]["selected_candidate_id"] == simt.candidate_id
    assert "bias_relu" in required["execution_plan_selection"]["kernel_symbol"]
    assert "bias_relu" in required["execution_plan_selection"]["profiler_symbol"]
...
def test_cutlass_conv_bias_add_relu_static_execution_plan_updates_manifest_and_conv_plan():
    ...
    workloads = build_profile_workloads(
        dml.trace(
            ...
            name="profile_conv2d_bias_add_relu_static_plan",
        ).ir,
        kernel_manifest,
    )
    simt = next(item for item in workloads if item.candidate["selection_predicate"]["kind"] == "fallback")
    ...
    assert plan["summary"]["static_selection_count"] == 1
    assert plan["static_selections"][0]["kernel_library"] == "cutlass_conv"
    assert plan["static_selections"][0]["op"] == "conv2d_bias_add_relu"
    assert plan["static_selections"][0]["selected_candidate_id"] == simt.candidate_id
    assert "bias_add_relu" in plan["static_selections"][0]["kernel_symbol"]
    assert "bias_add_relu" in plan["static_selections"][0]["profiler_symbol"]
    ...
    assert required["profiler_symbol"] == simt.profiler_symbol
    assert required["execution_plan_selection"]["selected_candidate_id"] == simt.candidate_id
    assert "bias_add_relu" in required["execution_plan_selection"]["kernel_symbol"]
    assert "bias_add_relu" in required["execution_plan_selection"]["profiler_symbol"]
...
def test_profile_artifact_writes_cutlass_conv_report_cache_and_execution_plan(tmp_path, monkeypatch):
    spec = dml.trace(
        ...
        name="profile_artifact_conv2d_bias",
    )
    kernel_manifest = build_kernel_manifest(spec.ir, DEFAULT_CUDA_TARGET)
    codegen_plan = create_codegen_plan(kernel_manifest, tmp_path / "cache").to_json()
    artifact = tmp_path / "profile_artifact_conv2d_bias.dinoml"
    artifact.mkdir()
    manifest = {
        "schema_version": 1,
        "target": DEFAULT_CUDA_TARGET,
        "files": {
            ...
            "runtime_library": "libdinoml_runtime.so",
            "cuda_runtime_library": "libdinoml_cuda_runtime.so",
        },
    }
    write_json(artifact / "manifest.json", manifest)
    write_json(artifact / "graph.dinoir.json", spec.ir)
    ...
    class FakeCudaProfiler:
        ...
        def profile(self, workload, *, iterations, rng):
            ...
            return elapsed, int(workload.workspace_nbytes)

        def close(self):
            pass

    monkeypatch.setattr(profiling_mod, "_CudaProfiler", FakeCudaProfiler)

    report = profile_artifact(artifact, iterations=7, repeats=3, refresh=True)
    cache = read_json(profile_cache_path(codegen_plan))
    execution_plan = read_json(artifact / "debug" / "execution_plan.json")

    assert report["summary"] == {"profiled": 2, "cached": 0, "skipped": 0, "failed": 0}
    assert report["problems"][0]["kernel_library"] == "cutlass_conv"
    assert all(problem["shape"]["n"] == 2 for problem in report["problems"])
    assert len(cache["entries"]) == 2
    assert {entry["kernel_library"] for entry in cache["entries"].values()} == {"cutlass_conv"}
    assert all("alignment_context" not in entry for entry in cache["entries"].values())
    assert report["execution_plan"]["static_selection_count"] == 1
    assert execution_plan["summary"]["static_selection_count"] == 1
    assert execution_plan["static_selections"][0]["kernel_library"] == "cutlass_conv"
    assert execution_plan["static_selections"][0]["selected_candidate_id"].endswith("simt_sm80_nhwc_ohwi_bias")


def test_compile_consumes_static_cutlass_conv_execution_plan_for_lowering(tmp_path, monkeypatch):
    calls = []
    ...
    spec = dml.trace(
        ...
        name="compile_profile_selected_conv2d_bias",
    )
    ...
    execution_plan = {
        ...
        "static_selections": [
            {
                "selection_key": "profile-selected-conv",
                "kernel_library": "cutlass_conv",
                "op": "conv2d_bias",
                "dtype": "float16",
                "candidate_set_key": required["candidate_set_key"],
    ...
    artifact = dml.compile(
        spec,
        target,
        tmp_path / "compile_profile_selected_conv2d_bias.dinoml",
...
def test_compile_consumes_static_cutlass_conv_bias_relu_execution_plan_for_lowering(tmp_path, monkeypatch):
    calls = []
    ...
    spec = dml.trace(
        ...
        name="compile_profile_selected_conv2d_bias_relu",
    )
    target = dml.Target("cuda", arch="sm_86")
    base_manifest = build_kernel_manifest(spec.ir, target.to_json())
    required = base_manifest["required_kernels"][0]
    ...
    execution_plan = {
        ...
        "static_selections": [
            {
                "selection_key": "profile-selected-conv-bias-relu",
                "kernel_library": "cutlass_conv",
                "op": "conv2d_bias_relu",
                "dtype": "float16",
                "candidate_set_key": required["candidate_set_key"],
    ...
    artifact = dml.compile(
        spec,
        target,
        tmp_path / "compile_profile_selected_conv2d_bias_relu.dinoml",
        execution_plan=execution_plan,
    )
    kernel_manifest = read_json(artifact.path / "kernel_manifest.json")
    codegen_plan = read_json(artifact.path / "kernel_codegen_plan.json")
...
def test_compile_consumes_static_cutlass_conv_bias_add_relu_execution_plan_for_lowering(tmp_path, monkeypatch):
    calls = []
    ...
    spec = dml.trace(
        ...
        name="compile_profile_selected_conv2d_bias_add_relu",
    )
    target = dml.Target("cuda", arch="sm_86")
    base_manifest = build_kernel_manifest(spec.ir, target.to_json())
    required = base_manifest["required_kernels"][0]
    ...
    execution_plan = {
        ...
        "static_selections": [
            {
                "selection_key": "profile-selected-conv-bias-add-relu",
                "kernel_library": "cutlass_conv",
    ...
    artifact = dml.compile(
        spec,
        target,
        tmp_path / "compile_profile_selected_conv2d_bias_add_relu.dinoml",
        execution_plan=execution_plan,
    )
    kernel_manifest = read_json(artifact.path / "kernel_manifest.json")
    codegen_plan = read_json(artifact.path / "kernel_codegen_plan.json")
...
def test_cuda_profile_artifact_writes_cutlass_conv_bias_relu_report_and_execution_plan(
    ...
    spec = dml.trace(
        ...
        name="profile_conv2d_bias_relu",
    )
    artifact = dml.compile(spec, dml.Target("cuda", arch="sm_86"), tmp_path / "profile_conv2d_bias_relu.dinoml")
    report = profile_artifact(artifact.path, iterations=3, repeats=3, refresh=True)
    ...
    low_confidence_count = int(report["execution_plan"]["low_confidence_count"])
    assert (selection_count, low_confidence_count) in {(1, 0), (0, 1)}
    assert {problem["op"] for problem in report["problems"]} == {"conv2d_bias_relu"}
    assert {problem["kernel_library"] for problem in report["problems"]} == {"cutlass_conv"}
    assert {problem["candidate"]["epilogue"] for problem in report["problems"]} == {"bias_relu"}
    assert all(problem["candidate"]["epilogue_config"] == {"inputs": ["bias"], "activation": "relu"} for problem in report["problems"])
    assert all(problem["candidate"]["launch_abi"] == "dinoml_cutlass_conv2d_bias_relu_v1" for problem in report["problems"])
    assert all(problem["selected"]["candidate_id"] == problem["candidate_id"] for problem in report["problems"])
    assert all("bias_relu" in problem["kernel_symbol"] for problem in report["problems"])
    assert all("bias_relu" in problem["profiler_symbol"] for problem in report["problems"])
    assert int(execution_plan["summary"]["selection_count"]) == selection_count
    assert int(execution_plan["summary"]["low_confidence_count"]) == low_confidence_count
    if selection_count:
        assert int(report["execution_plan"]["static_selection_count"]) == 1
        ...
        selected = execution_plan["low_confidence_selections"][0]
    assert selected["kernel_library"] == "cutlass_conv"
    assert selected["op"] == "conv2d_bias_relu"
    assert "bias_relu" in selected["kernel_symbol"]
    assert "bias_relu" in selected["profiler_symbol"]
...
def test_cuda_profile_artifact_writes_cutlass_conv_bias_add_report_and_execution_plan(
    ...
    spec = dml.trace(
        ...
        name="profile_conv2d_bias_add",
    )
    artifact = dml.compile(spec, dml.Target("cuda", arch="sm_86"), tmp_path / "profile_conv2d_bias_add.dinoml")
    report = profile_artifact(artifact.path, iterations=3, repeats=3, refresh=True)
    ...
    low_confidence_count = int(report["execution_plan"]["low_confidence_count"])
    assert (selection_count, low_confidence_count) in {(1, 0), (0, 1)}
    assert {problem["op"] for problem in report["problems"]} == {"conv2d_bias_add"}
    assert {problem["kernel_library"] for problem in report["problems"]} == {"cutlass_conv"}
    assert {problem["candidate"]["epilogue"] for problem in report["problems"]} == {"bias_add"}
    assert all(problem["candidate"]["epilogue_config"] == {"inputs": ["bias", "d0"]} for problem in report["problems"])
    assert all(problem["candidate"]["launch_abi"] == "dinoml_cutlass_conv2d_bias_add_v1" for problem in report["problems"])
    assert all(problem["candidate"]["candidate_id"] == problem["candidate_id"] for problem in report["problems"])
    assert all(problem["selected"]["candidate_id"] == problem["candidate_id"] for problem in report["problems"])
    assert all(problem["selected"]["reason"] == "only_candidate" for problem in report["problems"])
    assert all(problem["kernel_symbol"].startswith("dinoml_cutlass_conv2d_bias_add_float16_") for problem in report["problems"])
    assert all(problem["profiler_symbol"].startswith("dinoml_profile_cutlass_conv2d_bias_add_float16_") for problem in report["problems"])
    assert int(execution_plan["summary"]["selection_count"]) == selection_count
    assert int(execution_plan["summary"]["low_confidence_count"]) == low_confidence_count
    if selection_count:
        assert int(report["execution_plan"]["static_selection_count"]) == 1
        ...
        selected = execution_plan["low_confidence_selections"][0]
    assert selected["kernel_library"] == "cutlass_conv"
    assert selected["op"] == "conv2d_bias_add"
    assert selected["selected_candidate_id"] in {problem["candidate_id"] for problem in report["problems"]}
    assert selected["kernel_symbol"].startswith("dinoml_cutlass_conv2d_bias_add_float16_")
    assert selected["profiler_symbol"].startswith("dinoml_profile_cutlass_conv2d_bias_add_float16_")
...
def test_cuda_profile_artifact_writes_cutlass_conv_bias_add_relu_report_and_execution_plan(
    ...
    spec = dml.trace(
        ...
        name="profile_conv2d_bias_add_relu",
    )
    artifact = dml.compile(spec, dml.Target("cuda", arch="sm_86"), tmp_path / "profile_conv2d_bias_add_relu.dinoml")
    report = profile_artifact(artifact.path, iterations=3, repeats=3, refresh=True)
    ...
    low_confidence_count = int(report["execution_plan"]["low_confidence_count"])
    assert (selection_count, low_confidence_count) in {(1, 0), (0, 1)}
    assert {problem["op"] for problem in report["problems"]} == {"conv2d_bias_add_relu"}
    assert {problem["kernel_library"] for problem in report["problems"]} == {"cutlass_conv"}
    assert {problem["candidate"]["epilogue"] for problem in report["problems"]} == {"bias_add_relu"}
    assert all(
        problem["candidate"]["epilogue_config"] == {"inputs": ["bias", "d0"], "activation": "relu"}
        for problem in report["problems"]
    )
    assert all(
        problem["candidate"]["launch_abi"] == "dinoml_cutlass_conv2d_bias_add_relu_v1"
        for problem in report["problems"]
    )
    assert all(problem["candidate"]["candidate_id"] == problem["candidate_id"] for problem in report["problems"])
    assert all(problem["selected"]["candidate_id"] == problem["candidate_id"] for problem in report["problems"])
    assert all(problem["selected"]["reason"] == "only_candidate" for problem in report["problems"])
    assert all(
        problem["kernel_symbol"].startswith("dinoml_cutlass_conv2d_bias_add_relu_float16_")
        for problem in report["problems"]
    )
    assert all(
        problem["profiler_symbol"].startswith("dinoml_profile_cutlass_conv2d_bias_add_relu_float16_")
        for problem in report["problems"]
    )
    assert int(execution_plan["summary"]["selection_count"]) == selection_count
    assert int(execution_plan["summary"]["low_confidence_count"]) == low_confidence_count
    if selection_count:
        ...
        selected = execution_plan["low_confidence_selections"][0]
    assert selected["kernel_library"] == "cutlass_conv"
    assert selected["op"] == "conv2d_bias_add_relu"
    assert selected["selected_candidate_id"] in {problem["candidate_id"] for problem in report["problems"]}
    assert selected["kernel_symbol"].startswith("dinoml_cutlass_conv2d_bias_add_relu_float16_")
    assert selected["profiler_symbol"].startswith("dinoml_profile_cutlass_conv2d_bias_add_relu_float16_")
