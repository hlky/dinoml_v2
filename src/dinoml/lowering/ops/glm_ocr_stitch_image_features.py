...
from dinoml.ops.glm_ocr import GLM_OCR_STITCH_IMAGE_FEATURES_DTYPES, normalize_glm_ocr_stitch_image_features_shapes


def render_generated_kernel(target: str, node: Mapping[str, Any], tensor_map: Mapping[str, Mapping[str, Any]]) -> str:
    spec = supported_target_spec(target, "glm_ocr_stitch_image_features")
    context = _context(target, node, tensor_map)
    if spec.is_gpu:
        context.update(spec.gpu_template_context())
    template_name = "glm_ocr_stitch_image_features_gpu.j2" if spec.is_gpu else "glm_ocr_stitch_image_features_cpu.cpp.j2"
...
def source_key(target: str, node: Mapping[str, Any], tensor_map: Mapping[str, Mapping[str, Any]]) -> str:
    supported_target_spec(target, "glm_ocr_stitch_image_features")
...
def _context(target: str, node: Mapping[str, Any], tensor_map: Mapping[str, Mapping[str, Any]]) -> dict[str, Any]:
    ...
    image_token_id = _validate_node_contract(node, input_ids_tensor, inputs_embeds_tensor, image_features_tensor, output_tensor)
...
def _validate_node_contract(
    ...
    if node["op"] != "glm_ocr_stitch_image_features":
        raise ValueError(f"Unsupported GLM-OCR op: {node['op']}")
    if len(node.get("inputs", [])) != 3:
        raise ValueError("glm_ocr_stitch_image_features expects three inputs")
    if len(node.get("outputs", [])) != 1:
        raise ValueError("glm_ocr_stitch_image_features expects exactly one output")
    dtype = str(output_tensor["dtype"])
    if dtype not in GLM_OCR_STITCH_IMAGE_FEATURES_DTYPES:
        raise NotImplementedError(f"glm_ocr_stitch_image_features lowering does not support dtype {dtype}")
    if str(inputs_embeds_tensor["dtype"]) != dtype or str(image_features_tensor["dtype"]) != dtype:
        raise ValueError("glm_ocr_stitch_image_features data inputs and output must share dtype")
    if str(input_ids_tensor["dtype"]) not in {"int64", "int32"}:
        raise ValueError("glm_ocr_stitch_image_features input_ids must have dtype int64 or int32")
    ...
    if list(output_tensor["shape"]) != list(inputs_embeds_tensor["shape"]):
        raise ValueError("glm_ocr_stitch_image_features output shape must match inputs_embeds")
    attrs = node.get("attrs", {})
    image_token_id = attrs.get("image_token_id")
    if not isinstance(image_token_id, int) or isinstance(image_token_id, bool):
        raise ValueError("glm_ocr_stitch_image_features image_token_id must be an integer attr")
...
def _function_name(node: Mapping[str, Any], tensor_map: Mapping[str, Mapping[str, Any]]) -> str:
    ...
    output_tensor = tensor_map[str(node["outputs"][0])]
    signature = {
        "op": "glm_ocr_stitch_image_features",
    ...
    return f"glm_ocr_stitch_image_features_{digest}"


GLM_OCR_STITCH_IMAGE_FEATURES_LOWERING = OpLowering(
    op_name="glm_ocr_stitch_image_features",
