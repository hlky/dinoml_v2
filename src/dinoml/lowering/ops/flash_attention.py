...
def render_launch(
    ...
    if op_name == "flash_attention_varlen":
        return _render_varlen_launch(target, node, tensor_map, kernel_manifest)
...
def _render_varlen_launch(
    ...
    if target != "rocm":
        raise ValueError(f"flash_attention_varlen lowering is only implemented for ROCm, got {target!r}")
    q_name, k_name, v_name, cu_name = (str(name) for name in node["inputs"])
    output_name = str(node["outputs"][0])
    q_ident = _c_ident(q_name)
    k_ident = _c_ident(k_name)
    v_ident = _c_ident(v_name)
    cu_ident = _c_ident(cu_name)
    output_ident = _c_ident(output_name)
    q_shape = list(tensor_map[q_name]["shape"])
    k_shape = list(tensor_map[k_name]["shape"])
    v_shape = list(tensor_map[v_name]["shape"])
    cu_shape = list(tensor_map[cu_name]["shape"])
    output_shape = list(tensor_map[output_name]["shape"])
    if any(len(shape) != 3 for shape in (q_shape, k_shape, v_shape, output_shape)):
        raise ValueError("flash_attention_varlen expects rank-3 q/k/v/output tensors")
    if len(cu_shape) != 1:
        raise ValueError("flash_attention_varlen expects rank-1 cu_seqlens")
    dtype = str(tensor_map[output_name]["dtype"])
    if dtype not in {"float16", "bfloat16"}:
        raise ValueError(f"flash_attention_varlen {target} lowering only supports float16 and bfloat16, got {dtype}")
    if str(tensor_map[cu_name]["dtype"]) != "int32":
        raise ValueError(f"flash_attention_varlen cu_seqlens must be int32, got {tensor_map[cu_name]['dtype']}")
    causal = node.get("attrs", {}).get("causal", False)
    if not isinstance(causal, bool):
        raise TypeError("flash_attention_varlen causal attr must be a bool")
    max_seqlen = node.get("attrs", {}).get("max_seqlen")
    ...
    return "\n".join(
        [
            f'if (shape_{cu_ident}_0 <= 1) '
            'return dinoml::module::fail("flash_attention_varlen cu_seqlens length must be at least 2");',
            f'if (shape_{q_ident}_0 != shape_{k_ident}_0 || shape_{q_ident}_0 != shape_{v_ident}_0) '
            'return dinoml::module::fail("flash_attention_varlen total sequence dimension mismatch");',
            f'if (shape_{k_ident}_0 != shape_{v_ident}_0 || shape_{k_ident}_1 != shape_{v_ident}_1 || '
            f'shape_{k_ident}_2 != shape_{v_ident}_2) '
            'return dinoml::module::fail("flash_attention_varlen key/value shape mismatch");',
            f'if (shape_{q_ident}_2 != shape_{k_ident}_2) '
            'return dinoml::module::fail("flash_attention_varlen head_dim mismatch");',
            f'if ((shape_{q_ident}_1 % shape_{k_ident}_1) != 0) '
            'return dinoml::module::fail("flash_attention_varlen head grouping mismatch");',
            f'if (shape_{q_ident}_2 <= 0 || shape_{q_ident}_2 > 256) '
            'return dinoml::module::fail("flash_attention_varlen unsupported head_dim");',
            f'if (shape_{output_ident}_0 != shape_{q_ident}_0 || shape_{output_ident}_1 != shape_{q_ident}_1 || '
            f'shape_{output_ident}_2 != shape_{q_ident}_2) '
            'return dinoml::module::fail("flash_attention_varlen output shape mismatch");',
            f"if (int err = {symbol}(ptr_{q_ident}, ptr_{k_ident}, ptr_{v_ident}, ptr_{cu_ident}, ptr_{output_ident}, "
            f"static_cast<int64_t>(shape_{q_ident}_0), static_cast<int64_t>(shape_{cu_ident}_0 - 1), "
            f"static_cast<int64_t>({int(max_seqlen)}), static_cast<int64_t>(shape_{q_ident}_1), "
            f"static_cast<int64_t>(shape_{k_ident}_1), static_cast<int64_t>(shape_{q_ident}_2), "
            f'{causal_arg}, session->stream)) return dinoml::module::fail("flash_attention_varlen launcher failed");',
        ]
    )
...
def _render_static_kv_cache_launch(
    ...
    if target == "cuda":
        cuda_advance_arg = f", {1 if advance_cache_seqlens else 0}"
    ...
    return "\n".join(
        [
            ...
            *bias_checks,
            ...
            f"static_cast<int64_t>(shape_{q_ident}_3){bias_dim_args}{cuda_advance_arg}{rocm_scratch_args}, session->stream)) "
...
FLASH_ATTENTION_VARLEN_LOWERING = OpLowering(
    op_name="flash_attention_varlen",
    render_generated_kernel=render_generated_kernel,
    render_launch=render_launch,
)
