...
from dinoml.kernels.providers.ck.gemm import ck_gemm_candidates
...
def main() -> None:
    ...
    profiler_symbols = sorted({candidate["profiler_symbol"] for candidate in ck_gemm_candidates(args.op, args.dtype)})
    lines = [
    ...
    lines.extend(
        [
            '  throw std::runtime_error("CK GEMM profiler symbol not found: " + symbol);',
            "}",
            "",
            "}  // namespace dinoml::ck_gemm_profiler",
            "",
