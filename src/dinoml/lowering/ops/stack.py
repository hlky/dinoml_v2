...
from dinoml.ops.collections import COLLECTION_DTYPES, normalize_stack_dim, resolve_stack_shape
from dinoml.lowering.shape_buffers import c_ident as _c_ident


def render_generated_kernel(target: str, node: Mapping[str, Any], tensor_map: Mapping[str, Mapping[str, Any]]) -> str:
    spec = supported_target_spec(target, "stack")
    context = _context(target, node, tensor_map)
    if not spec.is_gpu:
        return _render_template("stack_cpu.cpp.j2", context)
    context.update(spec.gpu_template_context())
    return _render_template("stack_gpu.j2", context)
...
def source_key(target: str, node: Mapping[str, Any], tensor_map: Mapping[str, Mapping[str, Any]]) -> str:
    supported_target_spec(target, "stack")
...
def _validate_node_contract(
    ...
    if len(node.get("outputs", [])) != 1:
        raise ValueError("stack expects exactly one output")
    dtype = str(output_tensor["dtype"])
    if dtype not in COLLECTION_DTYPES:
        raise NotImplementedError(f"stack lowering does not support dtype {output_tensor['dtype']}")
    if any(str(tensor["dtype"]) != dtype for tensor in input_tensors):
        raise ValueError("stack input and output dtype must match")
    dim = normalize_stack_dim(node.get("attrs", {}).get("dim", 0), len(input_tensors[0]["shape"]))
    expected_shape = resolve_stack_shape([tensor["shape"] for tensor in input_tensors], dim)
    if list(expected_shape) != list(output_tensor["shape"]):
        raise ValueError("stack output shape does not match input shapes")
    return dim
...
def _copy_body(input_tensors: list[Mapping[str, Any]], output_tensor: Mapping[str, Any], dim: int) -> str:
    ...
    for axis in output_shape[dim + 1 :]:
        inner *= int(axis)
    stack_extent = len(input_tensors)
    lines = [
        f"  const int64_t inner = {inner};",
        f"  const int64_t stack_extent = {stack_extent};",
        "  const int64_t inner_idx = idx % inner;",
        "  const int64_t stack_idx = (idx / inner) % stack_extent;",
        "  const int64_t outer_idx = idx / (inner * stack_extent);",
    ]
    for index, _tensor in enumerate(input_tensors):
        prefix = "if" if index == 0 else "else if"
        lines.append(f"  {prefix} (stack_idx == {index}) {{")
        lines.append(f"    y[idx] = x{index}[outer_idx * inner + inner_idx];")
        lines.append("  }")
    return "\n".join(lines)
