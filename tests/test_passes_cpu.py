...
def test_apply_execution_plan_rejects_low_confidence_guarded_dispatch():
    ...
    spec = dml.trace(
        BmmModel(),
        inputs={"a": dml.TensorSpec([2, 4, 8], "float32"), "b": dml.TensorSpec([2, 8, 6], "float32")},
        name="bmm_guarded_low_confidence_manifest",
    )
    lowered, _ = PassManager().run(spec.ir)
    manifest = build_kernel_manifest(lowered, DEFAULT_CUDA_TARGET)
    required = manifest["required_kernels"][0]
    ...
    guarded_plan = {
        ...
        "selections": [
            {
                "selection_key": "low-confidence-guarded-selection",
                "node_id": lowered["nodes"][0]["id"],
                "op": "bmm_rrr",
                "dtype": "float32",
                "candidate_set_key": required["candidate_set_key"],
                ...
                "profiler_symbol": selected_candidate["profiler_symbol"],
                "shape": {"m": 4, "n": 6, "k": 8, "batch_count": 2},
                "confidence": {"confident": False, "level": "low"},
                "split_k": 1,
                "workspace_nbytes": 0,
            }
        ],
...
def test_apply_execution_plan_rejects_low_confidence_static_selection():
    import dinoml as dml
    ...
    spec = dml.trace(
        GemmModel(),
        inputs={"a": dml.TensorSpec([4, 8], "float32"), "b": dml.TensorSpec([8, 6], "float32")},
        name="gemm_profile_low_confidence_static_manifest",
    )
    lowered, _ = PassManager().run(spec.ir)
    manifest = build_kernel_manifest(lowered, DEFAULT_CUDA_TARGET)
    required = manifest["required_kernels"][0]
    ...
    execution_plan = {
        ...
        "static_selections": [
            {
                "selection_key": "low-confidence-static-selection",
                "op": "gemm_rrr",
                "dtype": "float32",
                "candidate_set_key": required["candidate_set_key"],
                "selected_candidate_id": selected_candidate["candidate_id"],
