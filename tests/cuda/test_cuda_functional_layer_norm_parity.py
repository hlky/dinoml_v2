...
def test_cuda_functional_layer_norm_parity(case, tmp_path):
    torch = pytest.importorskip("torch")
    if not torch.cuda.is_available():
        pytest.skip("CUDA runtime is required")
    capability = torch.cuda.get_device_capability()
    if case.dtype == "float16" and capability < (6, 0):
        pytest.skip("float16 parity requires CUDA device support")
