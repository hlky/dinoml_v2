...
epilogues, BMM `_add` epilogues, grouped GEMM, and public `matmul` layout
selection remain follow-up work. Base `bmm_*` layouts now use a separate
`cutlass_bmm` support library and profile path with batch-count, batch-stride,
and leading-dimension ABI fields, v1 layout semantics, C-column output
handling, batch-broadcast strides, static profile-selected candidate
