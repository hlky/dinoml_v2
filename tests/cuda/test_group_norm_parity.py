...
class _CudaGroupNormParityModule(dml.Module):
...
def _trace_group_norm_parity_spec(op_name: str, dtype: str, shape: list[int], num_groups: int):
    ...
    return dml.trace(
        _CudaGroupNormParityModule(op_name, num_groups),
        inputs={
            "x": dml.TensorSpec(shape, dtype),
            "weight": dml.TensorSpec([channels], dtype),
            "bias": dml.TensorSpec([channels], dtype),
        },
        name=f"cuda_{op_name}_{dtype}_{'x'.join(str(dim) for dim in shape)}_g{num_groups}",
    )
...
def test_cuda_group_norm_parity_matches_torch(op_name: str, dtype: str, shape: list[int], num_groups: int, tmp_path):
    ...
    artifact = dml.compile(
        ...
        tmp_path / f"{op_name}_{dtype}_group_norm_parity_cuda.dinoml",
    )
