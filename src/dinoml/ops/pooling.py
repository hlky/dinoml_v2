...
def infer_avg_pool2d_shape_with_attrs(input_shapes: Sequence[Sequence[int]], attrs: Mapping[str, Any]) -> list[int]:
    ...
    return resolve_avg_pool2d_shape(
        input_shapes[0],
...
def normalize_avg_pool2d_attrs(
    ...
    kernel = _normalize_positive_pair(kernel_size, "avg_pool2d kernel_size")
    normalized_stride = kernel if stride is None else _normalize_positive_pair(stride, "avg_pool2d stride")
    normalized_padding = _normalize_non_negative_pair(padding, "avg_pool2d padding")
...
def resolve_avg_pool2d_shape(
    ...
    if len(input_shape) != 4:
        raise ValueError(f"avg_pool2d expects rank-4 NCHW input, got rank {len(input_shape)}")
    ...
    out_height = _pool_output_dim(height, kernel[0], normalized_stride[0], normalized_padding[0], "height")
    out_width = _pool_output_dim(width, kernel[1], normalized_stride[1], normalized_padding[1], "width")
    return [n, c, out_height, out_width]


def _pool_output_dim(dim: int, kernel: int, stride: int, padding: int, axis_name: str) -> int:
    output = (int(dim) + 2 * padding - kernel) // stride + 1
    if output <= 0:
        raise ValueError(
            f"avg_pool2d output {axis_name} must be positive; got input={dim}, "
