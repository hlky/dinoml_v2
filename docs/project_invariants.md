...
- Runtime and provider decisions should be artifact-visible.
...
- Runtime load, unload, and reload paths must fail clearly when state is invalid.
...
- reference validation or a documented reason it is unavailable
- backend lowering or documented bounded helper behavior
- tests or direct validation
- checklist or doc updates when the public or artifact-visible behavior changes

## Docs

- Any behavior change that affects architecture, ops, providers, runtime, constants, artifacts, or benchmark semantics must update the relevant docs.
