...
- [x] Base BMM layouts:
  ...
  fallback. `bmm_rcr` and `bmm_rrr` additionally have bounded compiled CPU
  bridges for CLIP attention: generated CPU artifacts keep rank-3 semantics,
...
- [ ] Convolution: `conv2d`, `conv3d`, `conv3d_bias`, `depthwise_conv3d`,
  ...
  lanes. One bounded residual-plus-activation slice is now also admitted as
  `conv2d_bias_add_relu`: it keeps that same public static rank-4 groups=1
  ...
  coverage on the float16 default path, and focused float32 SIMT CUDA runtime
  parity. Broader float32 TensorOp runtime, fp16 residual+ReLU runtime,
  bfloat16 Conv runtime, grouped/depthwise/transposed/3D Conv, guarded/dynamic
  Conv dispatch, and richer residual/add+activation Conv epilogues remain
  unported.
