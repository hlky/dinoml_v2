...
def _torch_case_fns() -> dict[str, Callable[..., Any]]:
    fns = {
        ...
        "clamp": lambda torch, x, **_: torch.clamp(x, min=-0.5, max=0.75),
        ...
        "randn_torch": lambda torch, device, **_: _randn(torch, (1024, 4096), device=device, seed=17),
        "randn_bfloat16": lambda torch, device, **_: _randn(torch, (1024, 4096), device=device, seed=17, dtype=torch.bfloat16),
        "randn_torch_bfloat16": lambda torch, device, **_: _randn(torch, (1024, 4096), device=device, seed=17, dtype=torch.bfloat16),
