...
_TRANSPOSED_CONV2D_CUDA_DTYPES = ("float16", "float32", "bfloat16")
_ATOL_BY_DTYPE = {"float16": 0.005, "float32": 0.01, "bfloat16": 0.03}
_RTOL_BY_DTYPE = {"float16": 0.003, "float32": 0.01, "bfloat16": 0.03}


class _CudaTransposeConv2dParityModule(dml.Module):
    def forward(self, x, weight):
        y = dml.ops.transposed_conv2d(
            x,
            weight,
            stride=(2, 1),
            padding=(1, 0),
...
@pytest.mark.parametrize("dtype", _TRANSPOSED_CONV2D_CUDA_DTYPES)
def test_cuda_transposed_conv2d_parity_matches_torch(dtype: str, tmp_path):
    ...
    spec = dml.trace(
        _CudaTransposeConv2dParityModule(),
        inputs={
            "x": dml.TensorSpec([1, 8, 3, 4], dtype),
            "weight": dml.TensorSpec([8, 16, 3, 2], dtype),
        },
        name=f"cuda_transposed_conv2d_{dtype}_parity",
    )
    ...
    artifact = dml.compile(
        ...
        tmp_path / f"transposed_conv2d_{dtype}_cuda_parity.dinoml",
    )
    ...
    expected = _torch_transposed_conv2d_oracle(torch, all_inputs, dtype=dtype)
...
def _random_inputs(dtype: str) -> dict[str, np.ndarray]:
    ...
    inputs = {
        "x": rng.standard_normal([1, 8, 3, 4], dtype=np.float32).astype(np.float32),
        "weight": rng.standard_normal([8, 16, 3, 2], dtype=np.float32).astype(np.float32),
    }
    if dtype == "float16":
...
def _torch_transposed_conv2d_oracle(torch, inputs: dict[str, np.ndarray], *, dtype: str) -> np.ndarray:
